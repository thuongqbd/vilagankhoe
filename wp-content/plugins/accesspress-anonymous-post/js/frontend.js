	...
	$(function() {
		...
		$('.ap-form-submit-button').click(function() {
			...
			else
			{
				...
				if ($('textarea[name="ap_form_post_title"]').val() == '')
				{
					var error_message = $('textarea[name="ap_form_post_title"]').attr('data-required-message');
					if (error_message == '')
					{
						error_message = ap_form_required_message;
					}
					
					$('textarea[name="ap_form_post_title"]').closest('.ap-form-field-wrapper').find('.ap-form-error-message').html(error_message);
				}
