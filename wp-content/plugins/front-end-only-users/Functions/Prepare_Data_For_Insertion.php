...
//		if (empty($_POST['User_Password'])) { $user_update = array("Message_Type" => "Error", "Message" => __("The password cannot be empty.", "EWD_FEUP")); return $user_update;}
		...
		if($Use_Crypt == "Yes") {
			if (empty($_POST['User_Password'])) {$User_Fields['User_Password'] = Generate_Password($_POST['User_Password']);}
		} else {
			if (empty($_POST['User_Password'])) {$User_Fields['User_Password'] = sha1(md5($_POST['User_Password'].$Salt));}
		}
		
		$wpdb->get_results($wpdb->prepare("SELECT User_ID FROM $ewd_feup_user_table_name WHERE Username='%s'", $_POST['Username']));
		if ($wpdb->num_rows > 0) {$user_update = array("Message_Type" => "Error", "Message" => __("Email này đã được sử dụng, vui lòng chọn email khác.", "EWD_FEUP")); return $user_update;}
...
function EWD_FEUP_Send_Email($User_Fields, $Additional_Fields_Array, $User_ID = 0) {
	...
	wp_mail($User_Email,$Email_Subject,$Message_Body,$headers);
...
function EWD_FEUP_Send_Admin_Registration_Email($User_Fields, $Additional_Fields_Array, $User_ID) {
	...
	$Email_Subject = "Người dùng hưởng ứng mới";
	$Message_Body = "Xin chào Admin, \n\r";
	$Message_Body .= "Bạn có một người hưởng ứng mới.\n\r";
	$Message_Body .= "- Username/Email: " . $User_Fields['Username'] . ",\n\r";
...
//	if ($Admin_Approval == "Yes") {$Message_Body .= "Your site currently requires admin approval of users, therefore you will need to approve this user before they are able to log in.\n\r";}
	$Message_Body .= "Vui lòng đăng nhập vào trang quản lý (". get_site_url() .") để xem thêm thông tin của người hưởng ứng mới!\n\r";
	
	$blogname = get_option('blogname');
	$email = get_option('admin_email');
	$headers = "MIME-Version: 1.0\r\n" . "From: ".$blogname." "."<".$email.">\n" . "Content-Type: text/HTML; charset=\"" . get_option('blog_charset') . "\"\r\n";
	wp_mail($Admin_Email,$Email_Subject,$Message_Body,$headers);
