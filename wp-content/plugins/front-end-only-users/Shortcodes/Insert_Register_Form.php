...
function Insert_Register_Form($atts) {
	...
	if (!isset($ConfirmationSuccess)) {	
		...
		$ReturnString .= "<div class='feup-pure-control-group left first'>";
		if($Username_Is_Email == "Yes") {
			...
			if (isset($_POST['Username'])) {
				$ReturnString .= "<input type='email' required class='ewd-feup-text-input' name='Username' value='" . $_POST['Username'] . "'>";
			} else {
				$ReturnString .= "<input type='email' required class='ewd-feup-text-input' name='Username' placeholder='" . __('Email', 'EWD_FEUP') . "...'>";
			}
		} else {
		...
		else {$ReturnString .= "<input type='text' required class='ewd-feup-text-input' name='Username' placeholder='" . __('Username', 'EWD_FEUP') . "...'>";}
		}
		$ReturnString .= "</div>";
...
//		$ReturnString .= "</div>";
		...
		foreach ($Fields as $Field) {
			...
			$ReturnString .= "<div class='feup-pure-control-group $class'>";
			if($class == 'right' || $class == 'right first')
				$class = 'left';
			else
				$class = 'right';
...
//			$ReturnString .= "<label for='" . $Field->Field_Name . "' id='ewd-feup-register-" . $Field->Field_ID . "' class='ewd-feup-field-label'>" . __($Field->Field_Name, 'EWD_FEUP') . ": </label>";
			...
			if ($Field->Field_Type == "mediumint") {
				if($Field->Field_ID == 4){
					$att = 'maxlength="2" min="1" max="99"';
				}elseif($Field->Field_ID == 5){
					$att = 'maxlength="9"';
				}else{
					$att = '';
				}
				if (isset($_POST[str_replace(" ", "_", $Field->Field_Name)])) {$ReturnString .= "<input name='" . $Field->Field_Name . "' id='ewd-feup-register-input-" . $Field->Field_ID . "' class='ewd-feup-text-input pure-input-1-3' type='number' ".$att." value='" . $_POST[str_replace(" ", "_", $Field->Field_Name)] . "' " . $Req_Text . "/>";}
				else {$ReturnString .= "<input name='" . $Field->Field_Name . "' id='ewd-feup-register-input-" . $Field->Field_ID . "' class='ewd-feup-text-input' type='number' ".$att."  placeholder='" . $Field->Field_Name . "' " . $Req_Text . "/>";}
			}
		...
		$ReturnString .= "<div class='line clearfix'></div><input type='submit' class='ewd-feup-submit feup-pure-button feup-pure-button-primary' name='Register_Submit' value=''>";
		$ReturnString .= "</form>";
		$ReturnString .= "</div>";
	}
	else {
